...
import type {
  Job, OverheadLine, OwnerDraw, JobStatus,
  ChangeOrder, LaborLine, MaterialLine, SubLine, EquipmentLine,
} from "./data";

type AnyRow = Record<string, unknown>;

function mapJob(row: AnyRow, kids: {
  cos: AnyRow[]; labor: AnyRow[]; mats: AnyRow[]; subs: AnyRow[]; equip: AnyRow[];
}): Job {
  return {
    id: String(row.id),
    ...
    changeOrders: kids.cos.map<ChangeOrder>((c) => ({
      ...
      amount: Number(c.amount), date: String(c.date),
    })),
    labor: kids.labor.map<LaborLine>((l) => ({
      id: String(l.id), worker: String(l.worker), role: String(l.role),
      hours: Number(l.hours), rate: Number(l.rate),
    })),
    materials: kids.mats.map<MaterialLine>((m) => ({
      id: String(m.id), item: String(m.item), qty: Number(m.qty),
      unit: String(m.unit), unitCost: Number(m.unit_cost),
    })),
    subs: kids.subs.map<SubLine>((s) => ({
      id: String(s.id), vendor: String(s.vendor),
      trade: s.trade as SubLine["trade"], amount: Number(s.amount),
    })),
    equipment: kids.equip.map<EquipmentLine>((e) => ({
      id: String(e.id), machine: String(e.machine),
      category: e.category as EquipmentLine["category"],
      days: Number(e.days), dayRate: Number(e.day_rate),
    })),
  };
}

async function loadAllJobs(supabase: any): Promise<Job[]> {
...
export const getOverhead = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<OverheadLine[]> => {
    ...
    return (data ?? []).map((r: AnyRow) => ({
      id: String(r.id), category: String(r.category),
      amount: Number(r.amount), period: String(r.period),
    }));
  });
...
export const getOwnerDraws = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<OwnerDraw[]> => {
    ...
    if (error) throw error;
    return (data ?? []).map((r: AnyRow) => ({
      id: String(r.id), owner: String(r.owner),
      amount: Number(r.amount), period: String(r.period),
    }));
  });
...
export const getFinancialsBundle = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<{ jobs: Job[]; overhead: OverheadLine[]; draws: OwnerDraw[] }> => {
