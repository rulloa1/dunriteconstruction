  ...
  notFoundComponent: () => (
    <AppShell title="Job not found">
      <p className="text-muted">
        ...
        <Link to="/app/jobs" className="text-blue underline">
          Back to jobs
        </Link>
        .
      </p>
    ...
    <AppShell
      ...
      actions={
        <>
          ...
          <Link to="/app/jobs" className="btn focus-ring">
    ...
    >
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        <KpiCard
          ...
          sub={
            job.closedDate
              ? `Closed ${new Date(job.closedDate).toLocaleDateString("en-US")}`
              : "In progress"
          }
        />
      </section>
      ...
      <section className="mt-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            onClick={() => dlg.openCreate("co")}
            className="btn !py-1.5 !px-2.5 focus-ring text-xs"
          >
            + Change order
          </button>
        </div>
        <div className="card overflow-hidden">
          <LineRow
            primary={<span className="font-semibold">Contract amount</span>}
            secondary="Base award (edit on job)"
            amount={job.contractAmount}
          />
      ...
      <section className="mt-8">
        ...
        <div className="grid gap-3 sm:gap-4">
          <CostSection
            title="Labor"
            subtotal={laborTotal(job.labor)}
            count={job.labor.length}
            onAdd={() => dlg.openCreate("labor")}
          >
            {job.labor.length === 0 && (
              <EmptyLine onAdd={() => dlg.openCreate("labor")} label="No labor lines yet" />
            )}
            {job.labor.map((l) => (
              <LineRow
                key={l.id}
                primary={
                  <>
                    <span className="font-semibold">{l.worker}</span>{" "}
                    <span className="text-dim">· {l.role}</span>
                  </>
                }
          ...
          <CostSection
            title="Materials"
            subtotal={materialsTotal(job.materials)}
            count={job.materials.length}
            onAdd={() => dlg.openCreate("materials")}
          >
            {job.materials.length === 0 && (
              <EmptyLine onAdd={() => dlg.openCreate("materials")} label="No material lines yet" />
            )}
            {job.materials.map((m) => (
              <LineRow
                key={m.id}
          ...
          <CostSection
            ...
            subtotal={subsTotal(job.subs)}
            count={job.subs.length}
            onAdd={() => dlg.openCreate("subs")}
          >
            {job.subs.length === 0 && (
              <EmptyLine onAdd={() => dlg.openCreate("subs")} label="No subcontractor lines yet" />
            )}
            {Object.entries(subsByTrade).map(([trade, lines]) => (
              <div key={trade}>
                <div
                  className="px-4 sm:px-5 py-2 kbd-label"
                  style={{ background: "var(--bg-elev)" }}
                >
          ...
          <CostSection
            title="Equipment"
            subtotal={equipmentTotal(job.equipment)}
            count={job.equipment.length}
            onAdd={() => dlg.openCreate("equipment")}
          >
            {job.equipment.length === 0 && (
              <EmptyLine onAdd={() => dlg.openCreate("equipment")} label="No equipment lines yet" />
            )}
      ...
      <section className="mt-8">
        ...
        <div className="card p-5 sm:p-6">
          <div className="grid sm:grid-cols-4 gap-4 items-end">
            ...
            <ReconCell
              label="= Gross profit"
              value={fmtUSD(t.grossProfit)}
              tone={t.margin >= 0.08 ? "gold" : "negative"}
              big
            />
            <ReconCell
              label="Margin"
...
function EmptyLine({ label, onAdd }: { label: string; onAdd: () => void }) {
  return (
    <div className="px-4 sm:px-5 py-6 text-center text-dim font-ui text-sm">
      {label} ·{" "}
      <button onClick={onAdd} className="text-blue hover:underline">
        add the first one
      </button>
    </div>
  );
