...
const OVERHEAD_FIELDS: Field[] = [
  {
    ...
    required: true,
    full: true,
    placeholder: "e.g. Insurance, Rent, Fuel",
  },
...
export function PeriodLineFormDialog({
  ...
  const m = useMutation({
    mutationFn: async (v: Record<string, string>) => {
      ...
      if (kind === "overhead") {
        return upOver({
          data: { id, category: v.category, amount: Number(v.amount), period: v.period },
        });
      }
...
export function usePeriodDialogs() {
  ...
  return {
    ...
    openDelete: (kind: PeriodKind, id: string, label: string) =>
