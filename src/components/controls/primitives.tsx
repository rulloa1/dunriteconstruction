...
/* ---------------- StatusBadge ---------------- */
const GREEN_CLS =
  "bg-[color-mix(in_oklch,var(--positive)_18%,transparent)] text-positive border-[color-mix(in_oklch,var(--positive)_45%,transparent)]";
const BLUE_CLS =
  "bg-[color-mix(in_oklch,var(--brand-blue)_14%,transparent)] text-blue border-[color-mix(in_oklch,var(--brand-blue)_45%,transparent)]";
const GOLD_CLS =
  "bg-[color-mix(in_oklch,var(--brand-gold)_14%,transparent)] text-gold border-[color-mix(in_oklch,var(--brand-gold)_45%,transparent)]";
const RED_CLS =
  "bg-[color-mix(in_oklch,var(--negative)_12%,transparent)] text-negative border-[color-mix(in_oklch,var(--negative)_50%,transparent)]";
...
export function StatusBadge({ status, label }: { status: string; label?: string }) {
  ...
  return (
    <span className={`pill ${v.cls}`} style={{ borderWidth: 1, borderStyle: "solid" }}>
      {label ?? v.label}
    </span>
  );
}
...
export function OverdueBadge() {
  return (
    <span
      ...
      style={{
        color: "var(--negative)",
        borderColor: "color-mix(in oklch, var(--negative) 50%, transparent)",
...
export function DataCard({
  ...
  return (
    <div className="card overflow-hidden">
      {(title || action) && (
        <div
          className="flex items-center justify-between px-4 sm:px-5 py-4 gap-3 border-b"
    ...
    <nav className="kbd-label flex items-center gap-2 flex-wrap" aria-label="Breadcrumb">
      {items.map((it, i) => (
        <span key={i} className="flex items-center gap-2">
          {it.to ? (
            <Link to={it.to} className="hover:text-blue focus-ring">
              {it.label}
            </Link>
          ) : (
