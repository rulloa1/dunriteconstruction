...
(function () {
      ...
      function (entries) {
        entries.forEach(function (e) {
          ...
          if (e.isIntersecting) {
            ...
            try {
              v.playbackRate = window.__vrate || 0.6;
            } catch (_) {}
            var p = v.play();
  ...
  function runPreloader(done) {
    ...
    var seen = false;
    try {
      seen = sessionStorage.getItem(PRE_KEY) === "1";
    } catch (_) {}
    ...
    function finish() {
      ...
      finished = true;
      try {
        sessionStorage.setItem(PRE_KEY, "1");
      } catch (_) {}
  ...
  function setupScroll() {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: function (self) {
        if (sb) sb.style.width = (self.progress * 100).toFixed(2) + "%";
      },
    });
    ...
    var imgs = frames.map(function (f) {
      return f.querySelector("video, img");
    });
    var vids = frames.map(function (f) {
    ...
    if (reduced) {
      ...
      for (var i = 1; i < N; i++) {
        var pos = (i - 1) / segs;
        tl.to(frames[i], { opacity: 1, duration: 1 / segs, ease: "power1.inOut" }, pos).fromTo(
          imgs[i],
          { scale: 1.16 },
          { scale: 1.0, duration: 1 / segs, ease: "none" },
          pos,
        );
      }
      ...
      gsap.to(track, {
        ...
        scrollTrigger: {
          ...
          start: "top top",
            ...
            return "+=" + (track.scrollWidth - window.innerWidth + window.innerHeight);
          },
          scrub: 1,
          pin: ".cap-pin",
          anticipatePin: 1,
